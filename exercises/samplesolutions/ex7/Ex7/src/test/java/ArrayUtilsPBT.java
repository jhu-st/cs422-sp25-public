...
public class ArrayUtilsPBT {
    ...
    Arbitrary<Integer[]> arrProviderAllZeroNeg() {
        Arbitrary<Integer> integerArbitrary = Arbitraries.integers().between(Integer.MIN_VALUE, -1);
        return integerArbitrary
                .array(Integer[].class).ofMinSize(0).ofMaxSize(6);
    }
    ...
    @Property
    boolean countR1R2PositiveArrayNegative(@ForAll("arrProviderAllZeroNeg") Integer[] arr, @ForAll @IntRange(min = 0, max = 0) int r1 , @ForAll @Positive int r2) {
        Assume.that(r1 < r2);
        return ArrayUtils.countOutsideRange(arr, r1, r2) == arr.length;
    }
